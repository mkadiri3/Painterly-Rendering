    ...
    def surface_to_canvas(self):
        ...
        if self.surface.get_format() == cairo.FORMAT_RGB24:
            ...
            surface_data = np.dstack((
                ...
                surface_data[:,:,0]))
            #print surface_data
            self.canvas = Image.fromarray(surface_data,mode='RGB')
            #result = Image.fromarray((surface_data * 255).astype(np.uint8))
            #result.save('out.png')
...
#                    print max_coord
    ...
    def calc_stroke(self, stroke, radius):
        ...
        x = np.uint8(stroke.points[0][0])
        y = np.uint8(stroke.points[0][1])
        #print 'stroke'
        #print stroke.points[0][1]
        #print 'y'
        #print y
        ...
        for stroke_length in range(1, MAX_STROKE_LEN+1):
            ...
            x = np.uint8(abs(floor(x+radius*dx)))
            y = np.uint8(abs(floor(y+radius*dy)))
            #print 'changed'
            #print y
